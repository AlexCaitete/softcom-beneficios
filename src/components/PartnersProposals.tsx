    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[60] p-6">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 px-4 py-2 border-2 border-gray-300 rounded-lg hover:bg-gray-50 transition-colors font-semibold">
            Cancelar
          </button>
          <button onClick={onConfirm} className="flex-1 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors font-semibold">
            Recusar
          </button>
        </div>
      </div>
...
export function PartnersProposals() {
  const [proposals, setProposals] = useState<Proposal[]>([
    {
      ...
      discountValue: '20%',
      description: 'Desconto válido para toda linha de notebooks profissionais. Parceria exclusiva para funcionários de alto desempenho.',
    ...
    },
    {
      ...
      discountValue: 'R$ 300',
      description: 'Plano premium com acesso completo a todas as aulas e equipamentos. Inclui avaliação física gratuita.',
    ...
    },
    {
      ...
      discountValue: '15%',
      description: 'Desconto aplicável em toda plataforma de cursos profissionalizantes e certificações.',
    ...
    },
    {
      ...
      discountValue: 'R$ 250',
      description: 'Voucher para jantar completo no restaurante premiado. Inclui entrada, prato principal e sobremesa.',
    ...
    },
    {
      ...
      discountValue: '30%',
      description: 'Desconto em pacotes nacionais e internacionais. Válido para reservas até dezembro.',
    ...
    },
    {
      ...
      discountValue: '25%',
      description: 'Desconto em serviços de manutenção preventiva e corretiva. Peças originais garantidas.',
  ...
  const confirmReject = () => {
    if (rejectingId !== null) {
      setProposals(proposals.map(p => p.id === rejectingId ? { ...p, status: 'recusado' } : p));
      setShowModal(false);
      setRejectingId(null);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600">Gerencie ofertas de descontos para seus funcionários</p>
          </div>
            ...
            return (
              <div
                ...
                className={`bg-white border-2 rounded-xl p-6 transition-all hover:shadow-lg ${
                  proposal.status === 'pendente' ? 'border-[#FFD700]' : 'border-gray-200'
                }`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold">{proposal.partnerName}</h3>
                      <p className="text-xs text-gray-500">{proposal.category}</p>
                    </div>
                  </div>
                  <span className={`px-2 py-1 ${statusStyle.bg} ${statusStyle.text} rounded-full text-xs font-semibold`}>
                    {statusStyle.label}
                  </span>
                </div>

                <div className="mb-4">
                  <h4 className="font-bold text-lg mb-2">{proposal.discountTitle}</h4>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-bold text-[#FFD700]">{proposal.discountValue}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Package className="w-4 h-4 text-gray-500" />
                    <span className="text-gray-600">Mínimo:</span>
                    <span className="font-bold">{proposal.minPoints} pontos</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Calendar className="w-4 h-4 text-gray-500" />
                    <span className="text-gray-600">Válido até:</span>
                    <span className="font-bold">{new Date(proposal.validUntil).toLocaleDateString('pt-BR')}</span>
                  </div>
                </div>
      ...
      {showModal && selectedProposal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="bg-black text-white p-6 rounded-t-2xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold">{selectedProposal.partnerName}</h3>
            ...
            <div className="p-6">
              <div className="mb-6">
                <h4 className="text-2xl font-bold mb-3">{selectedProposal.discountTitle}</h4>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-2xl font-bold text-[#FFD700]">{selectedProposal.discountValue}</p>
                </div>

                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Package className="w-5 h-5 text-gray-600" />
                    <span className="text-sm text-gray-600">Pontos Mínimos</span>
                  </div>
                  <p className="text-2xl font-bold">{selectedProposal.minPoints}</p>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-lg font-bold">{new Date(selectedProposal.validUntil).toLocaleDateString('pt-BR')}</p>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-lg font-bold">{new Date(selectedProposal.receivedDate).toLocaleDateString('pt-BR')}</p>
                </div>
              </div>
              ...
              {selectedProposal.status !== 'pendente' && (
                <div className={`p-4 rounded-lg ${getStatusStyle(selectedProposal.status).bg} ${getStatusStyle(selectedProposal.status).text} text-center font-semibold`}>
